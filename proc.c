...
// now copy user space from the pinfo structure
...
found:
  ...
  //2. STRIDE SCHEDULER
  p->runtime = 0; //Initializing the runtime to 0
  //This ensures the new process is ok to run
...
scheduler(void)
{
  ...
  for(;;){
    ...
    struct proc *next_p = 0; //process with lowest runtime
    int min_runtime = 0x7FFFFFFF; //maximum integer value
    ...
    for(p = proc; p < &proc[NPROC]; p++) {
      ...
      if(p->state == RUNNABLE) {
        ...
        //Find the process with the lowest runtime
      ...
      //Release the lock after checking these fields
      release(&p->lock);
    }

    if(next_p) //If a candidate is found, switch to the next process
    {
      acquire(&next_p->lock); //reacquire the lock on this process before any changes being made
